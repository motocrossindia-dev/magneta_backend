...
from process_batch.models.batchMixTemplate import BatchMixTemplate, BatchMixTemplateIngredients
from process_batch.serializers.batchMixTemplateSerializer import \
    BatchMixTemplateSerializer, GETBatchMixTemplateSerializer, BatchMixTemplateUpdateSerializer
...
def batch_mix_templates(request):
    if request.method == 'GET':
        try:
            batch_mix_templates = BatchMixTemplate.objects.filter(is_deleted=False).prefetch_related(
                'ingredients')
...
def get_batch_mix_template(request, pk):
    if request.method == 'GET':
        try:
            get_batch_mix_templates = (BatchMixTemplate.objects.filter
                                   (pk=pk,is_deleted=False).prefetch_related('ingredients').first())
            if get_batch_mix_templates is None:
                return Response(status=status.HTTP_404_NOT_FOUND)
            serializer = GETBatchMixTemplateSerializer(get_batch_mix_templates)
            return Response({"data": serializer.data}, status=status.HTTP_200_OK)
        except Exception as e:
            print(e)
            return Response({'error': str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
@api_view(['PUT'])
def update_batch_mix_template(request, template_id):
    try:
        batch_template = BatchMixTemplate.objects.get(id=template_id)
    except BatchMixTemplate.DoesNotExist:
    ...
    serializer = BatchMixTemplateUpdateSerializer(batch_template, data=combined_data, partial=True)
