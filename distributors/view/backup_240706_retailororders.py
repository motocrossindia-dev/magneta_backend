...
def retailer_orders(request, pk=None):
    ...
    if request.method == 'GET' and (user.is_distributor or user.is_manager) and pk is not None:
        ...
        if main_order_serializer.is_valid():
            main_order = main_order_serializer.save(distributor=user, payment_status='Paid')
            if user.is_manager:
            ...
            for ordered_product_data in ordered_products:
                ...
                ordered_product_data['price_per_carton'] = round((product.factory_sale +
                                                                  product.distributor_margin_price) *
                                                                 product.carton_size, 2)
                ...
                if retailer.gst[:2] == "29":
                    ...
                    ordered_product_data['gst'] = (ordered_product_data['CGST'] + ordered_product_data['SGST'] +
