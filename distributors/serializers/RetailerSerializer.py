...
class GETretailerSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'user_id', 'first_name', 'phone_number', 'pan', 'gst', 'Address', 'email', 'pincode',
                  'enterprise_name', 'total_pending_amount']

    def get_total_pending_amount(self, obj):
        try:
            user = UserBase.objects.get(id=obj.id)
            total_pending_amount = RetailerMainOrders.objects.filter(retailer=user,payment_status="Unpaid").aggregate(total=Sum('pending_amount'))[
                                       'total'] or 0.00
